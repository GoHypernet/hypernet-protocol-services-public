...
async function main() {
	console.log("Seeding polygon Service Database");
	...
	new Promise((resolve, reject) => {
		connection.query(
			"CREATE USER 'polygon'@'%' IDENTIFIED WITH mysql_native_password BY 'polygon-password';",
			(err, res) => {
				if (err != null) {
					reject(err);
				}
	...
	await new Promise((resolve, reject) => {
		connection.query(
			"GRANT DELETE, INSERT, SELECT, UPDATE ON polygon.* TO 'polygon'@'%';",
			(err, res) => {
				if (err != null) {
					reject(err);
				}
