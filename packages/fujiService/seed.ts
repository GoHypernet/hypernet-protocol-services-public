...
async function main() {
	console.log("Seeding fuji Service Database");
	...
	new Promise((resolve, reject) => {
		connection.query(
			"CREATE USER 'fuji'@'%' IDENTIFIED WITH mysql_native_password BY 'fuji-password';",
			(err, res) => {
				if (err != null) {
					reject(err);
				}
	...
	await new Promise((resolve, reject) => {
		connection.query(
			"GRANT DELETE, INSERT, SELECT, UPDATE ON fuji.* TO 'fuji'@'%';",
			(err, res) => {
				if (err != null) {
					reject(err);
				}
