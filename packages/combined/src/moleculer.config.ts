...
import { MiddlewareOptions } from "@moleculer/channels/types/src";
import {
	BrokerOptions,
	Errors,
	MetricRegistry,
 ...
 *  }
 */
// const silenceNoise = process.env.SILENCE_NOISY_LOGS == "true";
const silenceNoise = true;
const brokerConfig: BrokerOptions = {
	// Namespace of nodes to segment your nodes on the same network.
	namespace: "Hypernet.ID",
	...
	metrics: {
		...
		reporter: {
			...
			options: {
				// Hostname
				host: "Combined Server",
				// Base URL
				baseUrl:
					process.env.DATADOG_BASE_URL ||
					"https://api.us5.datadoghq.com/", // Default is https://api.datadoghq.com/api/
				// API version
				apiVersion: "v1",
				// Server URL path
				path: "/series",
	...
	middlewares: [
		ChannelsMiddleware({
			adapter: process.env.CHANNELS_ADAPTER || "redis://redis:6379",
