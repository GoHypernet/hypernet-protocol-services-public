...
async function main() {
	console.log("Seeding avalanche Service Database");
	...
	new Promise((resolve, reject) => {
		connection.query(
			"CREATE USER 'avalanche'@'%' IDENTIFIED WITH mysql_native_password BY 'avalanche-password';",
			(err, res) => {
				if (err != null) {
					reject(err);
				}
	...
	await new Promise((resolve, reject) => {
		connection.query(
			"GRANT DELETE, INSERT, SELECT, UPDATE ON avalanche.* TO 'avalanche'@'%';",
			(err, res) => {
				if (err != null) {
					reject(err);
				}
