...
async function main() {
	console.log("Seeding mumbai Service Database");
	...
	new Promise((resolve, reject) => {
		connection.query(
			"CREATE USER 'mumbai'@'%' IDENTIFIED WITH mysql_native_password BY 'mumbai-password';",
			(err, res) => {
				if (err != null) {
					reject(err);
				}
	...
	await new Promise((resolve, reject) => {
		connection.query(
			"GRANT DELETE, INSERT, SELECT, UPDATE ON mumbai.* TO 'mumbai'@'%';",
			(err, res) => {
				if (err != null) {
					reject(err);
				}
